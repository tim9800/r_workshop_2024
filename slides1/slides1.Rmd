...
You then bind a value to the name using the `<-` operator

```{r}
my_object <- 123
```

To retrieve the value, we **call** the object by typing its name
...
`my_object + 1` is not an object, because it has no name
