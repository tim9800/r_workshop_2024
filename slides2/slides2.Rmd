...
```{r, eval = F}
ggplot(df,
       `aes`(x = sex_at_birth.factor)) + # Mapping
...
.box[
**How**: We *map* the x aesthetic to `sex_at_birth.factor`
...
## Sex by clinical stage

Instead of setting `fill` to a fixed value, lets make it so that the `fill` of 
each bar is mapped to the participant's clinical stage
...
ggplot(df,
       aes(x = sex_at_birth.factor)) + 
  geom_bar(
    aes(fill = clinical_stage.factor)) #<<
```

.box[
**How**: We *map* the `fill` aesthetic to `clinical_stage.factor`
