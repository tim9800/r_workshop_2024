...
Aesthetics can be applied in two ways:

* *Setting* - setting an aesthetic to a fixed value

* *Mapping* - attaching an aesthetic to a variable; the value
of the aesthetic *depends on* the value of the variable. General syntax:

```{r, eval = F}
ggplot(df,
       aes(aesthetic1 = some_variable_in_my_dataset,
           aesthetic2 = other_variable_in_my_dataset))
```


Aesthetics can be applied in two places:

* `ggplot()` - supplying aesthetics to the `ggplot()` function will propagate
those aesthetics to every subsequent layer

* Subsequent layers (e.g. `geom_()`) - supplying aesthetics at a specific layer
will apply those changes to that layer ONLY.

---

```{r}
ggplot(df,
       aes(x = sex_at_birth.factor, #<<
           fill = clinical_stage.factor)) + #<<
  geom_bar()
```
...
You can supply some aesthetics as **arguments** to `ggplot()`, and others as
aesthetics to the layers after that
